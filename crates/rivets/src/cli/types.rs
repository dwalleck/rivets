...
//! attributes all live on the domain declarations. This module keeps the
//! SortPolicyArg value enum and batch operation result types.
//!
//! `SortPolicyArg` mirrors the domain `SortPolicy` variant-for-variant, but
//! that domain type is not on the wire (no serde derives) and is not part of
//! the ADR-0004 vocabulary scope, so the CLI-side value enum stays here until
//! a domain twin earns serde/Display/FromStr of its own.
