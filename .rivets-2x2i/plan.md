...
- Apply the C0 named mutation without widening any visibility and run `cargo check -p rivets` → compilation fails with E0603 naming the private `storage::in_memory::inner` module; remove the mutation and the command returns green. Run the recorded AST/LSP seam check independently → every production graph mutation remains inside `storage::in_memory`, and any direct adapter call is named.
