...
| C6 | Persistence is canonical for new typed writes and preserves unrelated compatibility records. | Related add order; Discovery direction; restart; mixed kinds; legacy one-way/reciprocal Related | Perform typed mutations in opposite caller orders in equivalent Workspaces, save/reload twice, and compare raw relationship tuples and bytes; differing Related owner/order, reversed Discovery, dropped other kinds, or restart drift falsifies C6. Reciprocal legacy input is queried symmetrically but remains a migration input until `rivets-vio8`. | Independent raw JSONL parser normalizes literal `(record_owner,target,kind)` tuples and compares files without querying petgraph. | Store Related under the caller's first endpoint rather than canonical left endpoint; `related_and_discovery_persist_deterministically_across_restart` must report different raw owners for A/B versus B/A. | Resilient-loader test `related_and_discovery_persist_deterministically_across_restart`. | 2 minutes | PENDING — checkpointed-build, persistence slice |
