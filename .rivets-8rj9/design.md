...
- `2026-08-30 | cargo test -p rivets --test in_memory_storage assignment | PASS` — C1, C2, C3, C4, and C10 storage matrices agree with their independent tables; rejected operations leave snapshots/counts unchanged.
- `2026-08-30 | cargo test -p rivets --test in_memory_resilient_loading assignment_state_migration_is_visible_and_idempotent -- --exact | PASS` — C10 compatibility rows normalize visibly and the second canonical save is byte-stable.
- `2026-08-30 | cargo test -p rivets cli::tests:: | PASS` — C5 general update rejects Assignment flags and C6 Claim/Release parse and classify as Workspace mutations.
- `2026-08-30 | cargo test -p rivets --test cli_tests claim_release_cli_contract_survives_restart -- --exact | PASS` — C6 real-process success, idempotence, failure, readiness, release, close/reopen, and restart persistence agree with raw JSONL.
- `2026-08-30 | cargo test -p rivets --test workspace_mutation_lock synchronized_claims_have_one_durable_winner_and_terminal_retry -- --exact | PASS` — C8 synchronized claims produce one durable winner, unchanged owner retry, and terminal Already Claimed for losers.
- `2026-08-30 | cargo test -p rivets-mcp claim_release | PASS` — C7 public MCP tools preserve the storage outcomes and raw JSONL through context recreation.
- `2026-08-30 | cargo test -p rivets-mcp assignment_errors_preserve_retry_classification | PASS` — C11 protocol envelopes classify only Workspace Busy as retryable and preserve typed Assignment failures.
- `2026-08-30 | cargo test -p rivets-mcp --test workspace_lock | PASS` — C7/C9 every MCP mutator requires the durable lock, stale cache reload preserves mixed CLI/MCP writes, and close clears Assignment.
- `2026-08-30 | cargo test -p rivets-mcp --test workspace_lock workspace_lock_10k_mcp_mutation_preserves_records -- --ignored --exact --nocapture | PASS` — 10,000 guarded mutations completed in 62.916667 ms, inside the scale envelope.
- `2026-08-30 | cargo fmt --all -- --check && cargo clippy --all-targets --all-features -- -D warnings && cargo test --all-features | PASS` — final integrated regression fence passed 1,146 tests with 6 ignored.
