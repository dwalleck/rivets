...
Loading a legacy record is read-only: migration is applied in memory for inspection, while canonical JSONL is emitted only when a mutating operation saves the workspace.
