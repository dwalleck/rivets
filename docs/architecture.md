...
- **`rivets-mcp`**: MCP (Model Context Protocol) server exposing the tracker as 38 tools (the `crates/rivets-mcp` crate)
...
graph TB
    ...
    subgraph "MCP Layer (rivets-mcp)"
        Mcp[rivets-mcp server<br/>38 tools]
    end
...
- ✅ MCP server (`rivets-mcp`) with 38 tools, shared Workspace Information/Statistics, and per-call `workspace_root` overrides / `set_context` default workspace
