...
- **Find the next child of an Epic**: use MCP `ready` with `parent_id` and `limit: 1`, or `rivets ready --parent <epic-id> --limit 1 --json`. This selects direct children before ordering and limiting; it does not include grandchildren or claim the returned Issue. The parent must exist and be an Epic. An empty result means no child matches the current Ready query, not necessarily that the Epic is complete.
- **Update an issue**: use `update`, or `rivets update <issue-id>`, for title, description, Priority, Kind, design, and acceptance criteria. Supply at least one field. Assignment uses `claim` / `release`; Labels use atomic add/remove operations.
...
- **Record discussion or resolution**: use MCP `add_note` or CLI `rivets note append <issue-id> --content "..."`; each appends one immutable, system-timestamped Note.
- **Close or reopen**: use `close` / `reopen`, or the corresponding CLI commands, with a concise reason.
- **Start or return active work**: use MCP `start` / `return_to_open`, or CLI `rivets start <issue-id>` / `rivets return-to-open <issue-id>`. Starting requires Assignment; returning to Open retains it.
...
- **Frontier query**: query `ready` with `parent_id` set to the map Epic, or `rivets ready --parent <map-id> --kind task --limit <n> --json`. Add Kind or Label filters when needed. The default Assignment selector restricts the result to unassigned Ready Issues; use the tracker result order because Rivets has no explicit sub-issue order.
