...
**Complexity/production scale:** Create prerequisite validation is O(P), where P is requested prerequisites. Production expectation is fewer than 20; stress input is 10,000. Maximum accepted validation cost: 20 ms at 10,000 prerequisites, because validation is in-memory ID/duplicate lookup before one save. CLI tree delegates the O(V+E) work and budget to slice 1.  
