...
- Collections `notes`, `resources`, and `dependencies`: empty, one item, multiple distinct items, and repeated dependency values. Resource duplicate target/role combinations are rejected by the domain invariant rather than serialized.
...
| Contract tests | `crates/rivets-mcp/tests/integration.rs` exercises the public `Tools` seam and `Content::json`-equivalent serialization | Do not assert implementation-private conversion details; assert decoded JSON shape, ordering, omission, and timestamp values. |

No new interface or dependency is needed. The direct `Issue: Serialize` implementation already satisfies `Content::json`; MCP tool methods are internally typed and the server methods return `CallToolResult`.
